...
const CutterSidebar = () => {
    ...
    const fetchCounts = async () => {
      try {
        ...
        setTaskCounts({
          ...
          reassigned: data.reassigned?.length || 0,
        });
      } catch (err) {
  ...
  const menuItems = [
    ...
    { icon: Clock, label: "In Progress", path: "/cutter/cutterinprogress", count: taskCounts.inProgress },
    { icon: CheckCircle, label: "Completed", path: "/cutter/cuttercompleted", count: taskCounts.completed },
    { icon: ClipboardList, label: "Reassigned", path: "/cutter/cutterreassign", count: taskCounts.reassigned },
  ];
            ...
            <div className="flex items-center justify-between  md:px-10   relative">
              <img
                ...
                className="
                  ...
                  w-46 h-32  px-6 ml-6 m-4     /* Mobile */
                 
                  lg:w-80 lg:h-52  lg:px-4 lg:-m-2 lg:pt-2 lg:ml-4 /* Desktops */
                "
              />
            ...
            <nav className="px-3  -mt-6 flex-1">
              {menuItems.map(({ icon: Icon, label, path, count }) => (
                <NavLink
                  ...
                  to={path}
                  className={({ isActive }) =>
                    `group flex items-center px-4 py-3 mb-2 text-sm md:text-base font-medium rounded-xl transition-all duration-300 transform hover:scale-105
                    ${isActive
                  ...
                  {({ isActive }) => (
                    <>
                      ...
                      {count !== undefined && count > 0 && (
                        <span
                          className={`ml-auto bg-white/30 text-gray-800 text-xs md:text-sm font-semibold px-2 py-0.5 md:px-3 md:py-1 rounded-full`}
                        >
                          {count}
                        </span>
                      )}
