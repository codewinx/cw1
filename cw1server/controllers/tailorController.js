...
exports.getTailorTasks = async (req, res) => {
  try {
    ...
    if (!tailorId) {
      return res.status(400).json({ success: false, message: "tailor ID not found" });
    }

    const tasks = await Task.find({ assignedTo: tailorId })
      .populate({
        path: "order",
    ...
    console.error("Error fetching tailor tasks:", err.message);
...
exports.updateTailorTask = async (req, res) => {
  try {
    const { id } = req.params;
