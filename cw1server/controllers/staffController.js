...
exports.createStaff = async (req, res) => {
  try {
    ...
    const existingAadhar = await Staff.findOne({ aadharno });
    if (existingAadhar) {
      return res.status(400).json({ message: "Staff with this Aadhar number already exists" });
    }
...
exports.updateStaff = async (req, res) => {
  try {
    ...
    // Check for Aadhar number duplication, but only if it's being updated and different from the current one
    if (aadharno) {
      const existingAadhar = await Staff.findOne({ aadharno, _id: { $ne: req.params.id } });
      if (existingAadhar) {
        return res.status(400).json({ message: "Aadhar number already exists for another staff member" });
      }
    }
