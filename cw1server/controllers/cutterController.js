...
exports.getCutterTasks = async (req, res) => {
  try {
    ...
    if (!cutterId) {
      return res.status(400).json({ success: false, message: "Cutter ID not found" });
    }

    const tasks = await Task.find({ assignedTo: cutterId })
      .populate({
        path: "order",
    ...
    console.error("Error fetching cutter tasks:", err.message);
...
exports.updateCutterTask = async (req, res) => {
  try {
    const { id } = req.params;
